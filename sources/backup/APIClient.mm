...
- (void)forceExitApp:(NSString *)reason {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (rootVC) {
            ...
            id act = ((id(*)(id, SEL, id, NSInteger, void (^)(id)))objc_msgSend)(actionCls, actionSel, @"ออก", 2, ^(id action) {
                exitBlock();
            });
...
- (void)showMaintenanceAlert {
    ...
    Class alertCls = objc_getClass("UIAlertController");
    SEL alertSel = sel_registerName("alertControllerWithTitle:message:preferredStyle:");
    id alert = ((id(*)(id, SEL, id, id, NSInteger))objc_msgSend)(alertCls, alertSel, @"แจ้งเตือน", @"ขณะนี้ระบบกำลังอยู่ในระหว่างการบำรุงรักษา", 1);
    ...
    if (self.contactLink.length > 0) {
        id act = ((id(*)(id, SEL, id, NSInteger, void (^)(id)))objc_msgSend)(actionCls, actionSel, @"ติดต่อ", 2, ^(id action) {
            Class appCls = objc_getClass("UIApplication");
...
- (void)showKeyAlert {
    ...
    id actLogin = ((id(*)(id, SEL, id, NSInteger, void (^)(id)))objc_msgSend)(actionCls, actionSel, @"ล็อคอิน", 0, ^(id action) {
...
NSDate *expireDate = [df dateFromString:json[@"expiry"]];
            if ([expireDate timeIntervalSinceNow] < 0) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    ((void(*)(id, SEL, id, id, BOOL))objc_msgSend)(self, alertSel, @"เข้าสู่ระบบสำเร็จ", info, NO);
                });
                loop = NO;
            }
        }
    ...
    State500:
        {
            SEL alertSel = sel_registerName("showAlert:message:exit:");
            ((void(*)(id, SEL, id, id, BOOL))objc_msgSend)(self, alertSel, @"เกิดข้อผิดพลาดในการเชื่อมต่อ", error.localizedDescription, NO);
        }
        loop = NO;
        goto DispatchLoop;
    ...
    State501:
        {
            SEL alertSel = sel_registerName("showAlert:message:exit:");
            ((void(*)(id, SEL, id, id, BOOL))objc_msgSend)(self, alertSel, @"Server Error", @"ข้อมูลไม่ถูกต้อง", NO);
        }
        loop = NO;
        goto DispatchLoop;
...
- (void)handleInvalidKey:(NSString *)key message:(NSString *)msg isAuto:(BOOL)isAuto {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (isAuto) {
            ...
            SEL alertSel = sel_registerName("showAlert:message:exit:");
            ((void(*)(id, SEL, id, id, BOOL))objc_msgSend)(self, alertSel, @"Error", msg, YES);
        }
    });
}
...
- (void)showAlert:(NSString *)title message:(NSString *)message exit:(BOOL)retry {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.contactLink.length > 0) {
            id act = ((id(*)(id, SEL, id, NSInteger, void (^)(id)))objc_msgSend)(actionCls, actionSel, @"ติดต่อ", 2, contactHandler);
            ((void(*)(id, SEL, id))objc_msgSend)(alert, addActSel, act);
        }
        
        id actOK = ((id(*)(id, SEL, id, NSInteger, void (^)(id)))objc_msgSend)(actionCls, actionSel, @"ตกลง", 0, okHandler);
